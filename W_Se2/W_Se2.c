...
int diffusion(int x,int y , double local_event[10][2],int occ[row][col],float beta_ev){
    ...
    float rate=0;
    for(int a=0;a<6;a++){
        sxs=(x+row+dispx[a])%row;
        sys=(y+col+dispy[a])%col;

        if(NN(x,y,occ)==1 && NN(sxs,sys,occ)==1 && occ[sxs][sys]==0){
                rate = pow(10,12)*exp(-0.5*beta_ev)*0.5;   // 0.5 because same local_event is counted twice.
                local_event[0][0]=local_event[0][0]+rate;
                int c = local_event[0][1];
                point[0][c][0]=x;
                point[0][c][1]=y;
                point[0][c][2]=sxs;
                point[0][c][3]=sys;
        ...
        if(NN(x,y,occ)==1 && NN(sxs,sys,occ)==0 && occ[sxs][sys]==0){                                            //attachment of W/Se to the edge (Event 3/4)
            rate = pow(10,12)*exp(-2.5*beta_ev);   
            local_event[1][0]=local_event[1][0]+rate;
            int c = local_event[1][1];
            point[1][c][0]=x;
            point[1][c][1]=y;
            point[1][c][2]=sxs;
            point[1][c][3]=sys;
        ...
        if(NN(x,y,occ)==0 && NN(sxs,sys,occ)==0 && occ[sxs][sys]==0){                                              //surface diffusion of W/Se (Event 1/2)
            // printf("hello");
            rate = pow(10,12)*exp(-0.32*beta_ev);   // 0.5 because same local_event is counted twice.
            local_event[2][0]=local_event[2][0]+rate;
            int c = local_event[2][1];
            point[2][c][0]=x;
            point[2][c][1]=y;
            point[2][c][2]=sxs;
            point[2][c][3]=sys;
        ...
        if(NN(x,y,occ)==0 && NN(sxs,sys,occ)==1 && occ[sxs][sys]==0 ){          // attachment of W/Se to the edge (Event 3/4)
            rate = pow(10,12)*exp(-0.55*beta_ev);   // 0.5 because same local_event is counted twice.
            local_event[3][0]=local_event[3][0]+rate;
            int c = local_event[3][1];
            point[3][c][0]=x;
            point[3][c][1]=y;
            point[3][c][2]=sxs;
            point[3][c][3]=sys;
        ...
        if(NN(x,y,occ)==0 && NN(sxs,sys,occ)==2 && occ[sxs][sys]==0){
            rate = pow(10,12)*exp(-0.32*beta_ev);   // 0.5 because same local_event is counted twice.
            local_event[4][0]=local_event[4][0]+rate;
            int c = local_event[4][1];
            point[4][c][0]=x;
            point[4][c][1]=y;
            point[4][c][2]=sxs;
            point[4][c][3]=sys;
        ...
        if(NN(x,y,occ)==2 && NN(sxs,sys,occ)==0 && occ[sxs][sys]==0){
            rate = pow(10,12)*exp(-2.5*beta_ev);   // 0.5 because same local_event is counted twice.
            local_event[5][0]=local_event[5][0]+rate;
            int c = local_event[5][1];
            point[5][c][0]=x;
            point[5][c][1]=y;
            point[5][c][2]=sxs;
            point[5][c][3]=sys;
...
int NN(int x, int y, int occ[row][col]){
    ...
    stp=(y)%2+1;
    int m=0,n=0;                  // NN site around the diffusion site
    for (int b=0;b<3;b++){
            sxs=(x+row+ndispx[3*(stp-1)+b])%row;
            sys=(y+col+ndispy[3*(stp-1)+b])%col;  
            if (occ[sxs][sys]!=0){
            m++;
            swing[0]=sxs;
            swing[1]=sys;}
